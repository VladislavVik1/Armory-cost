...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Добавляем поле для комментария при открытии корзины
    document.querySelector(".cart-button")?.addEventListener("click", addCommentField);

    // Добавляем обработчик на кнопку отправки заказа
    document.addEventListener("click", function (event) {
        if (event.target.matches("#cart-modal button.snapshot")) {
            sendOrder();
        }
    });

    const socket = new WebSocket("wss://pmk-eagles.shop:8080"); // Подключаемся к серверу
